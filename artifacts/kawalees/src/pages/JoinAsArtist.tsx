...
async function uploadProfileImageToCloudinary(
  ...
  if (!cloudName || !uploadPreset) {
    throw new Error(
      "إعدادات رفع الصور غير مفعّلة بعد. أضف Cloudinary cloud name و unsigned upload preset قبل استقبال الطلبات الرسمية.",
    );
  }
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={onCancel}
              aria-label="إغلاق نافذة اقتصاص الصورة"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validate()) {
      toast({
        ...
        description: "تحقق من الحقول المطلوبة",
      });
      return;
    }
...
`,
      ...
      formData.append(
        "imageStatus",
        "تم رفع الصورة الشخصية إلى Cloudinary وإرفاق رابطها داخل الطلب",
      );
              ...
              <Section title="المعلومات الأساسية" icon={User}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="md:col-span-2">
                    <label htmlFor="artist-name" className={labelCls}>
                  ...
                  <div>
                    <label htmlFor="artist-country" className={labelCls}>
                      دولة الإقامة <span className="text-red-400">*</span>
                    </label>
                    <div className="relative">
                      <input
                        id="artist-country"
                      ...
                      <MapPin
                        size={15}
                        className="absolute top-1/2 right-3 -translate-y-1/2 text-gray-600 pointer-events-none"
                      />
                    </div>
                    {errors.country && (
                      <p className={errCls}>
                  ...
                  <div>
                    <label htmlFor="artist-date-of-birth" className={labelCls}>
                      تاريخ الميلاد <span className="text-red-400">*</span>
                    </label>
                    <input
                      id="artist-date-of-birth"
                      type="date"
                      value={dateOfBirth}
                    ...
                    {ageRange && (
                      <p className="mt-2 text-xs text-primary flex items-center gap-1.5">
                        ...
                        الفئة العمرية: <strong>{ageRange} سنة</strong>
                      </p>
                    )}
                    {errors.dateOfBirth && (
                      <p className={errCls}>
              ...
              <Section title="المعلومات الفنية" icon={Briefcase}>
                ...
                <div className="mb-6">
                  ...
                  {selectedSpecialties.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-3 p-3 bg-primary/5 rounded-xl border border-primary/10">
                      {selectedSpecialties.map((s) => (
                        <span
                          key={s}
                          className="flex items-center gap-1 pl-1 pr-2.5 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs"
                        >
                          {s}
                          <button
                            type="button"
                  ...
                  <div className="relative mb-3">
                    <input
                      id="artist-specialty-search"
                      type="text"
                      value={specialtySearch}
                      onChange={(e) => setSpecialtySearch(e.target.value)}
                      placeholder="ابحث عن تخصص..."
                      className={`${inputCls} pr-9 py-2 text-sm`}
                    />
                    <Search
                      size={14}
                      className="absolute top-1/2 right-3 -translate-y-1/2 text-gray-600 pointer-events-none"
                  ...
                  {specialtySearch &&
                    SPECIALTY_GROUPS.every((g) =>
                      g.items.every((i) => !i.includes(specialtySearch)),
                    ) && (
                      <p className="text-center text-gray-600 text-sm py-4">
                        لا توجد تخصصات مطابقة لـ "{specialtySearch}"
                      </p>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label htmlFor="artist-experience" className={labelCls}>
                      سنوات الخبرة <span className="text-red-400">*</span>
                    </label>
                    <div className="relative">
                      <select
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Link2
                        size={14}
                        className="absolute top-3.5 right-3 text-gray-600 pointer-events-none"
                      />
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white/3 border border-white/8 rounded-2xl p-6 sm:p-8">
              <Section title="بيانات التواصل" icon={Phone}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                  <div>
                    <label htmlFor="artist-phone" className={labelCls}>
                  ...
                  <div>
                    <label htmlFor="artist-email" className={labelCls}>
